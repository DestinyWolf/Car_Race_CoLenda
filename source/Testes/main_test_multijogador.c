...
//coisas a corrigir, aspectos de colisão do segundo jogador
//porque o jogo não continua apos a primeira checagem de colisão
...
pthread_mutex_t obstacle_1_mutex;
pthread_mutex_t obstacle_2_mutex;
...
pthread_cond_t obstacle_1_cond;
pthread_cond_t obstacle_2_cond;
...
int pause_background, pause_obstacle_1, pause_obstacle_2, pause_mouse_1, pause_mouse_2, pause_bullets, pause_colision;
...
void pause_threads() {
    ...
    printf("pause o mouse 2\n");
    pthread_mutex_lock(&obstacle_2_mutex);
    pause_obstacle_2 = 1;
    pthread_mutex_unlock(&obstacle_2_mutex);
    printf("pause obstacle 2\n");
    
    pthread_mutex_lock(&mouse_1_mutex);
    pause_mouse_1 = 1;
    pthread_mutex_unlock(&mouse_1_mutex);
    printf("pause o mouse 1\n");
    pthread_mutex_lock(&bullets_mutex);
    pause_bullets = 1;
    pthread_mutex_unlock(&bullets_mutex);
    printf("pause bullets\n");
    pthread_mutex_lock(&obstacle_1_mutex);
    pause_obstacle_1 = 1;
    pthread_mutex_unlock(&obstacle_1_mutex);
    printf("pause obstacle 1\n");
...
void reestart_threads() {
    ...
    pthread_mutex_unlock(&mouse_2_mutex);
    printf("voltou o mouse 2\n");
    pthread_mutex_lock(&obstacle_2_mutex);
    pause_obstacle_2 = 0;
    pthread_cond_signal(&obstacle_2_cond);
    pthread_mutex_unlock(&obstacle_2_mutex);
    printf("voltou os obstaculos 2\n");
    pthread_mutex_lock(&mouse_1_mutex);
    pause_mouse_1 = 0;
    
    pthread_cond_signal(&mouse_1_cond);
    pthread_mutex_unlock(&mouse_1_mutex);
    printf("voltou o mouse 1\n");
    pthread_mutex_lock(&bullets_mutex);
    pause_bullets = 0;
    pthread_cond_signal(&bullets_cond);
    pthread_mutex_unlock(&bullets_mutex);
    printf("voltou os disparos\n");
    pthread_mutex_lock(&obstacle_1_mutex);
    pause_obstacle_1 = 0;
    pthread_cond_signal(&obstacle_1_cond);
    pthread_mutex_unlock(&obstacle_1_mutex);
    printf("voltou o obstaculo 1\n");
    pthread_mutex_lock(&background_mutex);
    pause_background = 0;
...
void* random_obstacle_1_generate_routine(void* args) {
    while (1)
    {
        pthread_mutex_lock(&obstacle_1_mutex);
        while (pause_obstacle_1)
        {
            printf("ta pausado obstacle1\n");
            pthread_cond_wait(&obstacle_1_cond, &obstacle_1_mutex);
...
void* random_obstacle_2_generate_routine(void* args) {
    while (1)
    {
        pthread_mutex_lock(&obstacle_2_mutex);
        while (pause_obstacle_2)
        {
            printf("ta pausado obstacle2\n");
            pthread_cond_wait(&obstacle_2_cond, &obstacle_2_mutex);
...
void menu() {
    state = in_menu;
    int btn_val;
    if(pause_background || pause_bullets || pause_colision || pause_mouse_1 || pause_obstacle_1 || pause_mouse_2 && game_mode == dual_player || pause_obstacle_2 && game_mode == dual_player) {
        pause_threads();
    }
    while(1) {
        scanf("%d", &btn_val);
...
int main() {
    ...
    pthread_t obstacle_1_thread, obstacle_2_thread, background_thread, mouse_1_thread, mouse_2_thread, player_1_timer_thread, player_2_timer_thread, bullets_thread, colision_thread; 
    GPU_open();
    ...
    pthread_mutex_init(&obstacle_1_mutex, NULL);
    pthread_mutex_init(&obstacle_2_mutex, NULL);
    ...
    pthread_cond_init(&obstacle_1_cond, NULL);
    pthread_cond_init(&obstacle_2_cond, NULL);
    ...
    pthread_create(&obstacle_1_thread, NULL, random_obstacle_1_generate_routine, NULL);
    pthread_create(&obstacle_2_thread, NULL, random_obstacle_2_generate_routine, NULL);
    ...
    pthread_cancel(obstacle_1_thread);
    ...
    pthread_join(obstacle_1_thread, NULL);
    ...
    pthread_mutex_destroy(&obstacle_1_mutex);
    pthread_mutex_destroy(&obstacle_2_mutex);
    ...
    pthread_cond_destroy(&obstacle_1_cond);
    pthread_cond_destroy(&obstacle_2_cond);
